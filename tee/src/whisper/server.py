...
# so the attestation can be re-verified by anyone. In production the key is
# generated inside the TEE at boot and never exposed.
DEMO_TEE_SK = bytes.fromhex(os.getenv("TEE_SK", "11" * 32))
...
async def lifespan(app: FastAPI):
    ...
    engine = Engine(cfg)
    tee_pubkey_uncompressed = _derive_pubkey()
